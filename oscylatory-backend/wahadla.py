...
lengths = np.linspace(0.8, 1.2, num_pendulums)
natural_frequencies = np.sqrt(9.81 / lengths)
coupling_strength = 0.05
time_step = 0.05
num_steps = 400

# Inicjalizacja kątów i prędkości kątowych wahadeł
angles = np.random.uniform(-0.5, 0.5, num_pendulums)
angular_velocities = np.zeros(num_pendulums)
angle_history = []
...
def update(frame):
    ...
    for i, line in enumerate(lines):
        ...
        line.set_data(x, y)
        ax.plot(x, y, 'o-', lw=2, label=f'Wahadło {i + 1}')
